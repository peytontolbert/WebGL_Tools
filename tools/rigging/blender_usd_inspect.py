"""
USD stage inspection executed *inside Blender*.

Why:
- Many environments in this repo do not have `pxr` installed for system Python.
- Portable/upstream Blender builds typically ship OpenUSD Python bindings (`pxr`).

This script is designed to be called like:
  blender --background --factory-startup --python tools/rigging/blender_usd_inspect.py -- --in /abs/path/file.usd --top 30

It prints a single JSON object to stdout (same shape as tools/usd_inspect.py).
"""
...
def main(argv: list[str]) -> int:
    ...
    try:
        from pxr import Ar, Usd, UsdGeom, UsdSkel, UsdUtils  # type: ignore
    ...
    missing_files = sorted(list(dict.fromkeys([*missing_abs, *missing_resolved])))
